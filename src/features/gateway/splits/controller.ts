...
const entity = {
  model: 'SplitTransaction' as keyof PrismaClient,
  name: 'Splits',
  route: 'splits-transactions',
  permission: 'financeiro',
};

@Controller(entity.route)
export class SplitsController extends GenericController<
  CreateDto,
  UpdateDto,
  IEntity,
  Service
> {
  ...
  // @Cache({ prefix: 'splits-transactions', ttl: 3600 }) // descomente para usar cache (1 hora)
  @Get()
  ...
  // @Public() // descomente para tornar publica
  // @CacheEvictAll('splits-transactions:*', 'cache:*/splits-transactions*') // descomente para limpar cache
  @Post()
