...
export class ReviewsController extends GenericController<
  CreateDto,
  UpdateDto,
  IEntity,
  Service
> {
  ...
  // @Cache({ prefix: 'reviews', ttl: 3600 }) // descomente para usar cache (1 hora)
  @Get()
  ...
  @Post()
  @UseInterceptors(FileInterceptor('image', getMulterOptions('reviews-image')))
  ...
  // ROTA ABERTA PARA ALUNOS CADASTRAREM REVIEWS SEM NECESSIDADE DE PERMISSÃO
  @Public()
  @Post('create-review')
  ...
  async createReview(
    ...
    const search = getSearchParams(request, CreateDto);
    return this.Service.createReview(search, entity, CreateDto);
  }
}
