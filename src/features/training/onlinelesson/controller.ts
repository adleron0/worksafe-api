...
export class OnlinelessonController extends GenericController<
  CreateDto,
  UpdateDto,
  IEntity,
  Service
> {
  ...
  // @Cache({ prefix: 'online-lesson', ttl: 3600 }) // descomente para usar cache (1 hora)
  @Get()
  ...
  // @Public() // descomente para tornar publica
  // @CacheEvictAll('online-lesson:*', 'cache:*/online-lesson*') // descomente para limpar cache
  @Post()
  @UseInterceptors(
    FileInterceptor('image', getMulterOptions('online-lesson-image')),
  )
  @UsePipes(
    new ValidationPipe({
      whitelist: true,
