...
import { OnlinemodellessonService as Service } from './service';
...
const entity = {
  model: 'OnlineModelLesson' as keyof PrismaClient,
  name: 'Onlinemodellesson',
  route: 'online-course-lesson',
  permission: 'treinamentos',
};

@Controller(entity.route)
export class OnlinemodellessonController extends GenericController<
  CreateDto,
  UpdateDto,
  IEntity,
  Service
> {
  ...
  // @Cache({ prefix: 'online-course-lesson', ttl: 3600 }) // descomente para usar cache (1 hora)
  @Get()
  ...
  // @Public() // descomente para tornar publica
  // @CacheEvictAll('online-course-lesson:*', 'cache:*/online-course-lesson*') // descomente para limpar cache
  @Post()
  ...
  async upsert(@Req() request: Request, @Body() upsertDto: CreateDto) {
    const whereCondition = getUpsertWhereCondition(request, upsertDto);

    return super.upsert(request, upsertDto, null, whereCondition, hooksUpsert);
  }
}
