...
const fields = args.filter(arg => arg.startsWith('--field=')).map(arg => {
...
if (!entityNameArg) {
  ...
  console.log('Usage: node generate-entity.js <entityName> [--has-image] [--field=name:type:required]');
  console.log('Example: node generate-entity.js product --has-image --field=name:string:required --field=price:number:required');
  process.exit(1);
}
...
> {
  ...
  @Post()
  ${hasImage ? `@UseInterceptors(FileInterceptor('image', getMulterOptions('${entityName}-image')))` : ''}
  ...
  async create(
    ...
    return super.create(request, CreateDto${hasImage ? ', file' : ''}, search);
  }
  ...
  async update(
    ...
    return super.update(id, request, UpdateDto${hasImage ? ', file' : ''});
  }
...
fields.forEach(field => {
  ...
  createDtoContent += `
  ${decorators.join('\n  ')}
  ${field.name}: ${field.type === 'string' || field.type === 'email' || field.type === 'url' ? 'string' : 
                   field.type === 'number' || field.type === 'int' || field.type === 'float' ? 'number' : 
                   field.type === 'boolean' ? 'boolean' : 
                   field.type === 'date' ? 'Date' : 'string'};
`;
});
...
// Add fields from command line arguments (all optional for update)
fields.forEach(field => {
  const decorators = [];
